app.controller("EmployeeCtrl", ['$scope', '$http', '$location', '$routeParams', function ($scope, $http, $location, $routeParams) {
    ...
    $scope.Add = function () {
        var employeeData = {
            FirstName: $scope.City,
            LastName: $scope.Gender,
        ...
        debugger;
        $http.post("api/Employee/create", employeeData).success(function (data) {
            $location.path('/EmployeeList');
        }).error(function (data) {
            console.log(data);
    ...
    if ($routeParams.empId) {
        ...
        $http.get('api/Employee/getuser/' + $scope.Id).success(function (data) {
